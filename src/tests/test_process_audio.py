...
def test_split_audio() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for split in temp_dir_path.iterdir():
            ...
            assert abs(filesize - split.stat().st_size) <= 10, f"filesize differs by more than 10 bytes for {split}. found {split.stat().st_size}, expected {filesize}" #pylint: disable=line-too-long
