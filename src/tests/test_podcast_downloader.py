...
def test_download_episode_already_exists(mock_get, mock_test_post, downloader, app):
    with app.app_context():
        # Create the directory and file
        episode_dir = downloader.get_and_make_download_path(mock_test_post.title).parent
        episode_dir.mkdir(parents=True, exist_ok=True)
        episode_file = episode_dir / "Test Episode.mp3"
        episode_file.write_bytes(b"dummy data")

        result = downloader.download_episode(
            mock_test_post, dest_path=str(episode_file)
        )
...
@mock.patch("podcast_processor.podcast_downloader.requests.get")
def test_download_episode_new_file(mock_get, mock_test_post, downloader, app):
    with app.app_context():
        # Setup mock response
        ...
        expected_path = downloader.get_and_make_download_path(mock_test_post.title)
        result = downloader.download_episode(
            mock_test_post, dest_path=str(expected_path)
        )
...
@mock.patch("podcast_processor.podcast_downloader.requests.get")
def test_download_episode_download_failed(mock_get, mock_test_post, downloader, app):
    with app.app_context():
        # Setup mock response
        mock_response = mock.MagicMock()
...
def test_download_episode_invalid_url(
    mock_abort, mock_validator, mock_test_post, downloader, app
):
    with app.app_context():
        ...
        expected_path = downloader.get_and_make_download_path(mock_test_post.title)
        downloader.download_episode(mock_test_post, dest_path=str(expected_path))

        # Check that abort was called with 404
        mock_abort.assert_called_once_with(404)


def test_download_episode_invalid_post_title(mock_test_post, downloader, app):
    with app.app_context():
        # Test with a post that has an invalid title that results in empty sanitized title
        mock_test_post.title = "!@#$%^&*()"  # This will sanitize to empty string
