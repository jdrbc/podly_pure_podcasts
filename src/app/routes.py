    ...
    try:
        with app.app_context():  # Push application context
            ...
            if download_path is None:
                logger.error(f"Failed to download post: {post_in_thread.title} (ID: {post_in_thread.id})")
                result = {
            ...
            if output_path is None:
                logger.error(f"Failed to process post: {post_in_thread.title} (ID: {post_in_thread.id})")
                result = {
                    "post_id": post_in_thread.id,
                    "title": post_in_thread.title,
                    "status": "failed",
            ...
            logger.info(
                f"Successfully downloaded and processed post: {post_in_thread.title} (ID: {post_in_thread.id})"
            )
            result = {
                "post_id": post_in_thread.id,
                "title": post_in_thread.title,
        ...
        post_title = post.title if 'post_in_thread' not in locals() else post_in_thread.title
