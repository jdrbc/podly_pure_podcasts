...
def discord_config_put() -> Response | tuple[Response, int]:
    ...
    try:
        ...
        db_settings = db.session.get(DiscordSettingsModel, 1)
        if db_settings is None:
            db_settings = DiscordSettingsModel(id=1)
            db.session.add(db_settings)

        # Update fields (only if not overridden by env vars)
        if "client_id" in payload and not _has_env_override("DISCORD_CLIENT_ID"):
            db_settings.client_id = payload["client_id"] or None

        if "client_secret" in payload and not _has_env_override(
            "DISCORD_CLIENT_SECRET"
        ):
            # Only update if a new value is provided (not the masked preview)
            secret = payload["client_secret"]
            if secret and not secret.endswith("..."):
                db_settings.client_secret = secret

        if "redirect_uri" in payload and not _has_env_override("DISCORD_REDIRECT_URI"):
            db_settings.redirect_uri = payload["redirect_uri"] or None

        if "guild_ids" in payload and not _has_env_override("DISCORD_GUILD_IDS"):
            db_settings.guild_ids = payload["guild_ids"] or None
